...
\section*{``Solving'' NP-Hard Problems}
...
\begin{itemize}
    \item \textbf{Heuristics:} (with something like A*).  Not guaranteed to be optimal, but usually polynomial.
    \item \textbf{Approximation:} See below.
\end{itemize}
...
\subsubsection*{Example: Approximation of TSP}
...
\begin{itemize}
    \item Construct MST
    \item Use preorder traversal, and leave out repeated visits
\end{itemize}
How bad is this approximation?  Let $A$ be the edges visited on the tour by our algorithm, counting multiplicity.  Now, let $A^*$ be the edges visited in an optimal tour.  Let the function cost($S$) be the cost of a tour.\\\\
\textbf{Theorem:} cost($A$) $\leq 2$ cost($A^*$)
Let cost($T$) $\leq$ cost($A^*$).  A walk $w$ that traces the MST has length $2$cost($T$).  This is because every edge in the MST is visited twice.  Therefore, 
